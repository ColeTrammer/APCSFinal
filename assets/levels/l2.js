...
manager.spawn(new OneWayWall(
    p1Rect,
    new SimpleHarmonicOscillation(p1Rect, 1 /  2 * C.WORLD_WIDTH, C.WORLD_HEIGHT * FRACTION_CLOSED +     HEIGHT_INC, 1 / 5 * C.WORLD_WIDTH, 0, 5),
    Direction.DOWN));
manager.spawn(new OneWayWall(
    p2Rect,
    new SimpleHarmonicOscillation(p2Rect, 1 /  2 * C.WORLD_WIDTH, C.WORLD_HEIGHT * FRACTION_CLOSED + 2 * HEIGHT_INC, 1 / 5 * C.WORLD_WIDTH, 0, 5),
    Direction.DOWN));
manager.spawn(new OneWayWall(
    p3Rect,
    new SimpleHarmonicOscillation(p3Rect, 1 /  2 * C.WORLD_WIDTH, C.WORLD_HEIGHT * FRACTION_CLOSED + 3 * HEIGHT_INC, 1 / 5 * C.WORLD_WIDTH, 0, 5),
    Direction.DOWN));
manager.spawn(new OneWayWall(
    p4Rect,
    new SimpleHarmonicOscillation(p4Rect, 1 /  2 * C.WORLD_WIDTH, C.WORLD_HEIGHT * FRACTION_CLOSED + 4 * HEIGHT_INC, 1 / 5 * C.WORLD_WIDTH, 0, 5),
    Direction.DOWN));
